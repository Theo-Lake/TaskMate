...
export async function createTask(publisherId: number, data: unknown) {
    const res = await client.post(`/tasks/${publisherId}`, data);
    return res.data;
}
...
export async function applyForTask(taskId: number, userId: number) {
    const res = await client.post(`/tasks/${taskId}/apply/${userId}`);
    return res.data;
}
